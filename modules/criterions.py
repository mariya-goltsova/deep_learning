...
class MSELoss(Criterion):
    ...
    def compute_output(self, input: np.ndarray, target: np.ndarray) -> float:
        ...
        return np.mean(np.mean((target - input)**2, axis=1))
...
class CrossEntropyLoss(Criterion):
    ...
    def compute_output(self, input: np.ndarray, target: np.ndarray) -> float:
        ...
        # replace with your code ｀、ヽ｀、ヽ(ノ＞＜)ノ ヽ｀☂｀、ヽ
        # return super().compute_output(input, target)
        p = self.log_softmax.compute_output(input)
        return -np.mean(p[np.arange(target.shape[0]), target])
    ...
    def compute_grad_input(self, input: np.ndarray, target: np.ndarray) -> np.ndarray:
        ...
        # replace with your code ｀、ヽ｀、ヽ(ノ＞＜)ノ ヽ｀☂｀、ヽ
        # return super().compute_grad_input(input, target)
        p = np.exp(self.log_softmax.compute_output(input))
        p[np.arange(target.shape[0]), target] -= 1
        grad = p / target.shape[0]
        return grad
